...
AddonID='plugin.program.indigo'; AddonTitle=""
import os,xbmcaddon,xbmc,shutil
		...
		try:
			for (root, dirs, files) in os.walk(addonPath, topdown=True):
				...
				for file_name in files:
					try: os.remove(os.path.join(root, file_name))
					except:pass
			kodi.message(AddonTitle,"Done! , You are now back to a fresh Kodi configuration!","Click OK to exit Kodi and then restart to complete .")
