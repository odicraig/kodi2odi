...
from dudehere.routines.requests import RequestHandler
...
class PlaybackService(xbmc.Player):
	...
	def start(self):
		...
		#if plugin.get_setting('enable_background_caching') == 'true':
		#	from dudehere.routines import fanartservice
			
		#	self.log("Starting Caching Thread")
		
		if plugin.get_setting('enable_control_service') == 'true':
			CONTROL_PORT = int(plugin.get_setting('control_port'))
			if plugin.get_setting('network_bind') == 'Localhost':
				address = "127.0.0.1"
			else:
				address = "0.0.0.0"
			self.log("Launching Control WebInterface on: %s:%s" % (address, CONTROL_PORT))
