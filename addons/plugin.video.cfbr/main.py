'''
    College Football Replays Add-on
...
if mode == 'main':
    try:
        ...
        if playlists:
            for c in playlists:
                Addon.add_directory({'mode': c['title']}, c['title'])
    ...
    if mode == title:
        ...
        if streams:
            for c in streams:
                ...
                videoId = c['videoId']
                img = c['img']
                rURL = Addon.plugin_url + "?channel=" + channel + "&videoId=" + videoId + "&mode=play&rand=" + Addon.random_generator()
                Addon.add_video_item(rURL, {'title': title}, img=img)

    elif mode == 'play':
        videoId = Addon.plugin_queries['videoId']
        stream_status = source.Source()._get_json('/check.php', {'id': videoId})['status']
