...
class StreamsService(object):
    def __init__(self, addon):
        ...
        if int(addon.getSetting('addons.ini.type')) == 0:
            path = xbmc.translatePath(os.path.join('special://profile', 'addon_data', 'plugin.program.echotvguide', 'addons.ini'))
            xbmc.log('[plugin.program.echotvguide] addons.ini is used', xbmc.LOGDEBUG)
        else:
            customFile = str(addon.getSetting('addons.ini.file'))
            if os.path.exists(customFile):
                path = customFile
            else:
                path = xbmc.translatePath(os.path.join('special://profile', 'addon_data', 'plugin.program.echotvguide', customFile.split('/')[-1]))
            xbmc.log('[plugin.program.echotvguide] Custom addons.ini is used: %s' % path, xbmc.LOGDEBUG)
    ...
    def getIniFiles(self):
        ...
        files.append(os.path.join(datapath,'addons_paid.ini'))
    ...
    def detectStream(self, channel):
        ...
        for id in self.getAddons():
            ...
            for (label, stream) in self.getAddonStreams(id):
                ...
                #'''
                # New show any channel with keyword
                if id == "plugin.video.dex":#--mayfair check for dex and do a custom channel match
                    label = label.upper()
                    channel.title = channel.title.upper()
                    if label.startswith(channel.title):#--mayfair check channel starts with database name and show channel(better method for dexter so doesn't list wrong channels)
                        matches.append((id,label,stream))
                else:
                    label = label.upper()
                    channel.title = channel.title.upper()
