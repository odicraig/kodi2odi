import urllib,re,xbmcplugin,xbmcgui,xbmc,xbmcaddon,os,itertools
import urlresolver
import requests
...
baseurl50 =  'http://www.filmovizija.ws' #'http://www.filmovizija.studio/'  #'http://www.filmovizija.club' #'http://www.filmovizija.in'
baseurl150 = 'http://geektv.tv' #'http://geektv.is' #'http://geektv.me'
...
def INDEX():
        ...
        addDir('[COLOR cyan]Filmovizija[/COLOR]',baseurl50,50,art+'vizi.png',art+'f1.jpg','')
...
def BASE50INDEX(url):
        ...
        all_videos = regex_get_all(link, '<li><div id=', '</li>')
...
def BASE50HOSTS(url,iconimage):
        ...
        for url in sources:
                ...
                if urlresolver.HostedMediaFile(host=str(host), media_id='ABC123XYZ'):
                        addDir('[B][COLOR red]%s[/COLOR][/B]' %host.upper(),url,1,iconimage,art+'vizi.png','')
...
def BASE50INDEX2(url):
        ...
        for a in all_videos:
                ...
                url = baseurl50 + '/episode.php?vid='+url
                thumb = regex_from_to(a, "original='", "'")
                epn = regex_from_to(a, "epname'>", "<")
                if name > '':
...
def BASE150LINKS(name,url,iconimage,show_title):
        ...
        link = open_url(url)
        link = link.encode('ascii', 'ignore').decode('ascii')
        match=re.compile('<a href="(.*?)" class="btn btn-link btn-xs"><img src=".*?"> (.*?)</a></td>').findall(link)
        items = len(match)
        for url,name in match:
                if baseurl150 not in url :
                        url = baseurl150 + url
                link = open_url(url)
                link = link.encode('ascii', 'ignore').decode('ascii')
                match = re.findall(r'<a target="_blank" href="(.*?)" >watch link</a>', str(link), re.I|re.DOTALL)
                for url in match:
                ...
                if (keyb.isConfirmed()):
                        ...
                        try:
                                ...
                                for a in all_videos:
                                        ...
                                        addDir3('[B][COLOR white]%s[/COLOR][/B]' %name,baseurl150+url,156,thumb,items,'',name)
                        except:
...
def BASE150EP(name,url,iconimage,show_title):
        if baseurl150 not in url :
                        url = baseurl150 + url
        name = name.replace('[I][B][COLOR white]','').replace('[/COLOR][/B][/I]','')
        link = open_url(url)
        link = link.encode('ascii', 'ignore').decode('ascii')
        all_links = regex_get_all(link, 'href=".*?#collapse.*?">'+name, '</table>')
        all_videos = regex_get_all(str(all_links), '<tr><td><a', '</td></tr>')
        items = len(all_videos)
        for a in all_videos:
                name = regex_from_to(a, '</span>', '</')
                url = regex_from_to(a, 'href="', '"')
                thumb = regex_from_to(a, 'src="', '"')
                addDir3('[B][COLOR white]%s[/COLOR][/B]' %name,url,152,iconimage,items,'',show_title)
...
def BASE300(url):
        ...
        a = soup.findAll('li',class_='simple-links-item simple-links-widget-item')[:-3]
        for links in a:
                b = links.findAll('a', href=True)

                for cats in b:
                        if '%s/category/tv' %baseurl300 not in cats["href"]:
                                name = cats.get_text()
                                name = addon.unescape(name)
                                name = name.encode('ascii', 'ignore').decode('ascii')
                                url = cats ['href']

                                if 'Movies 4 Mobiles' not in name:
...
def BASE300INDEX(url,iconimage):

        link = open_url(url)
        soup = bs(link, 'html.parser')

        #a = soup.findAll('div', class_='boxitem')
        #if a == []:
        ...
        try:
                ...
                for links in a:
                        ...
                        addDir2(name,url,302,iconimage,items)
        except:
                pass
        try:
                all_videos = regex_get_all(link, '"boxitem"', '</div>')
                items = len(all_videos)
                for a in all_videos:
                        ...
                        addDir2(name,url,302,thumb,items)
        ...
        try:
                match=re.compile('<a class="next page-numbers" href="(.+?)">.+?</a>').findall(link)
                for url in match:
                        addDir('[I][B][COLOR cyan]>>Next Page>>>[/COLOR][/B][/I]',url,301,art+'ultra.png',art+'f1.jpg','')
        except: pass

        '''try:
                np = soup.find('a', href=True,  class_='next page-numbers')
                if np:
                        url = np["href"]
                        addDir('[I][B][COLOR cyan]>>Next Page>>>[/COLOR][/B][/I]',url,301,art+'ultra.png',art+'f1.jpg','')
        except: pass'''
...
def BASE300L(url,name,iconimage):

        link = open_url(url)
        soup = bs(link, 'html.parser')

        qual=re.compile('<strong>(.*?)</strong>').findall(str(soup))[-1]
        addLink('[B][COLOR cyan]%s[/COLOR][/B]' %qual,'url','',iconimage,art+'f1.jpg','')                

        sources = []

        a = soup.findAll('iframe')
        for links in a:
                sources.append(links['src'])

        b = soup.findAll('li', class_='alt')
        for links in b:
                c = links.findAll('a', href=True)
                for data in c:
                        sources.append(data['href'])
                        
        for url in sources:
                host = url.split('//')[1].split('/')[0].split('.')[0]
                if urlresolver.HostedMediaFile(host=str(host), media_id='ABC123XYZ'):
                        addDir('[B][COLOR white]%s[/COLOR][/B]' %host.upper(),url,1,iconimage,art+'f1.jpg','')
...
def BASE500INDEX(url):
        ...
        for a in all_videos:
                ...
                url = regex_from_to(a, '<iframe .+?src="', '"').replace("&amp;","&")
                if 'http:' not in url:
                        url = 'http:' + url
...
def addDir3(name,url,mode,iconimage,itemcount,description,show_title):
        if metaset=='true': 
                try:
                        ...
                        show_title = description.replace('[B][COLOR white]','').replace('[/COLOR][/B]','')
                try:
...
def open_url2(url):
    ...
    link = s.get(url,verify=False, headers=headers).text
    return link
...
if mode==None or url==None or len(url)<1:
        ...
        BASE150EP(name,url,iconimage,show_title)
        ...
        BASE300INDEX(url,iconimage)

elif mode==302:
        BASE300L(url,name,iconimage)
